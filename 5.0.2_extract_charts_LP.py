...
DATA_FOLDER = 'Data/Light_Positions_FFC/Results'
...
def process_image(image_path, working_dir, ref_illuminant, reference_rgb):
    ...
    try:
        ...
        for chart_index, chart in enumerate(charts, start=1):
            ...
            # log_(f'Chart_df - shape: {chart_df.shape}, \n\n{chart_df.head()}', "magenta", "italic", "warn")
            chart_df = chart_df.round(12)
        ...
        charts_csv_path = os.path.join(working_dir, f"{image_name}_Charts.csv")
        charts_df.to_csv(charts_csv_path, float_format='%.12f')
...
def main():
    ...
    for i, folder in enumerate(data_folders):
        working_dir = os.path.join(DATA_FOLDER, folder)
        if not os.path.exists(working_dir):
            log_(f"Folder does not exist: '{working_dir}'", "red", "italic", "warn")
            continue
        log_(f"Processing folder: {folder}, \t folder {i + 1}/{len(data_folders)}", "magenta", "italic")
