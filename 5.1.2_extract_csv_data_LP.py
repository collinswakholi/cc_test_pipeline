...
data_path = 'Data/Light_Positions_FFC/Results'
# data_path = 'Data/Backgrounds/Results'
search_str = '_Results_' 
save_str = search_str

def get_steps_deg_method(folder):
    
    if '_Side_' in folder:
        remap = re.sub(r'With_|_Side_|_Results_|Deg_', '#', folder)
        split_ = remap.split('#')
        return split_[1], split_[2], split_[3], split_[4]
    elif '_Top_' in folder:
        remap = re.sub(r'With_|_Top_|_Results_|Deg_', '#', folder)
        split_ = remap.split('#')
        return split_[1], 'Top_' + split_[2], split_[3], split_[4]
...
ALL_CHARTS_PD = pd.DataFrame()
for folder in tqdm(folders_selected):

    folder_basename = os.path.basename(folder)

    steps, Lamp_angle, deg, method = get_steps_deg_method(folder)

    first_op = steps.split('_')[0]
    log_(f'Processing {folder} with {first_op}', 'cyan', 'italic')
    ...
    charts_csv_files = [f for f in csv_files if '_Charts.csv' in f]
    ...
    for csv_file in Summary_metrics_csv_files:
        ...
        if first_op != 'FFC':
            # print(df.head(10))
            df = df.loc[~df.iloc[:, 0].str.contains('Before_') | df.iloc[:, 0].str.contains('Before_'+first_op)]
        else:
    ...
    # print(Summary_metrics_df.head(15))
    ...
    charts_pd = pd.DataFrame()
    for csv_file in charts_csv_files:
        file_base_folder = os.path.basename(os.path.dirname(csv_file))
        file_basename = os.path.basename(csv_file)
        file_basename = file_basename.replace('_Corrected_Charts.csv', '')
        df = None
        df = pd.read_csv(csv_file)
        if df.empty:
            log_(f'"{csv_file}" is empty', 'red', 'italic')
        ...
        df.insert(4, 'Deg', int(deg))
        df.insert(5, 'Method', method)
        charts_pd = pd.concat([charts_pd, df], axis=0)
    ...
    ALL_CHARTS_PD = pd.concat([ALL_CHARTS_PD, charts_pd], axis=0)


log_(f"Saving all metrics to '{os.path.join(data_path, 'All_Metrics.xlsx')}'", 'green', 'bold')
ALL_METRICS_PD.to_excel(os.path.join(data_path, 'All_Metrics.xlsx'), index=False, float_format='%.12f')

log_(f"Saving summary metrics to '{os.path.join(data_path, 'Summary_Metrics.xlsx')}'", 'green', 'bold')
SUMMARY_METRICS_PD.to_excel(os.path.join(data_path, 'Summary_Metrics.xlsx'), index=False, float_format='%.12f')

log_(f"Saving all metrics to '{os.path.join(data_path, 'All_Metrics_val.xlsx')}'", 'green', 'bold')
ALL_METRICS_VAL_PD.to_excel(os.path.join(data_path, 'All_Metrics_val.xlsx'), index=False, float_format='%.12f')  

log_(f"Saving summary metrics to '{os.path.join(data_path, 'Summary_Metrics_val.xlsx')}'", 'green', 'bold') 
SUMMARY_METRICS_VAL_PD.to_excel(os.path.join(data_path, 'Summary_Metrics_val.xlsx'), index=False, float_format='%.12f')

log_(f"Saving charts to '{os.path.join(data_path, 'Charts.xlsx')}'", 'green', 'bold')
ALL_CHARTS_PD.to_excel(os.path.join(data_path, 'Charts.xlsx'), index=False, float_format='%.12f')
