...
DATA_FOLDER = 'Data/Walls/Results' # for walls
# DATA_FOLDER = 'Data/Backgrounds/Results' # for background
# FOLDERS_TO_PROCESS = ['Black', 'White', 'Colors'] # for background
FOLDERS_TO_PROCESS = ['White_D65', 'Black_D65'] # for walls
# FOLDERS_TO_PROCESS = ['']
...
hex_colrs = get_color_chart_hex(REFERENCE_CHART, REF_ILLUMINANT)
hex_colrs = {name: hex_colrs[name] for name in names}
...
def process_image(image_path, working_dir, ref_illuminant, reference_rgb):
    ...
    try:
        ...
        columns=['Folder', 'Image', 'Chart', 'Tile', 'Hex', 'Ref_R', 'Ref_G', 'Ref_B', 'R', 'G', 'B']
        for chart_index, chart in enumerate(charts, start=1):
            chart_float = to_float64(chart)

            chart_df = pd.DataFrame(columns=columns)
            chart_df['Ref_R'] = REFERENCE_RGB[:, 0]
            chart_df['Ref_G'] = REFERENCE_RGB[:, 1]
            chart_df['Ref_B'] = REFERENCE_RGB[:, 2]
            chart_df['R'] = chart_float[:, 0]
            chart_df['G'] = chart_float[:, 1]
            chart_df['B'] = chart_float[:, 2]
            chart_df['Hex'] = [hex_colrs[name] for name in names]
            chart_df['Folder'] = base_folder
            chart_df['Image'] = image_name
            chart_df['Chart'] = chart_index
            chart_df['Tile'] = names

            charts_.append(chart_df)
        ...
        charts_df = pd.concat(charts_, ignore_index=False)
        ...
        charts_path = os.path.join(working_dir, f"{image_name}_Charts.csv")
        final_summary.to_csv(summary_csv_path, float_format='%.12f')
        final_all.to_csv(all_csv_path, float_format='%.12f')
        charts_df.to_csv(charts_path, float_format='%.12f')
...
def process_folder(working_dir):
    ...
    # remove any files that have 'Blank' in the name
    image_files = [f for f in image_files if 'Blank' not in f]
    if not image_files:
...
def main():
    ...
    for i, folder in enumerate(data_folders):
        for ii, f_x in enumerate(FOLDERS_TO_PROCESS):
            working_dir = os.path.join(DATA_FOLDER, folder, f_x)
            if not os.path.exists(working_dir):
                log_(f"Folder does not exist: '{working_dir}'", "red", "italic", "warn")
                continue
            log_(f"Processing folder: {folder} - {f_x}, \t folder {(i * len(FOLDERS_TO_PROCESS) + ii + 1)}/{len(data_folders) * len(FOLDERS_TO_PROCESS)}", "magenta", "italic")
