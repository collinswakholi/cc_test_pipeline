...
#---------------------------------------Configurations-------------------------------------------------------    
DATA_FOLDER = 'Data'
TEST_GROUP = 'Light_Positions_FFC'
...
SEQUENCES_ = [
    [False, True, True, True, True],  # sequence 1
    [True, True, True, True, True],  # sequence 2
]
...
LAMP_POSITIONS = [p for p in glob.glob(os.path.join(folder, '*')) if os.path.isdir(p) and any(f in p for f in FO_)]
...
def process_folder(f_):
    ...
    # Match folder name against the focus keywords
    matched_folder = match_keywords(os.path.basename(f_), FOCUS_ON, threshold=0.9)
    if matched_folder not in FOCUS_ON and 'All' not in FOCUS_ON:
        log_(f'Skipping Folder: "{matched_folder}"...', 'magenta', 'italic')
        return
    
    # matched_folder = os.path
    ...
    # split folder at /
    split_ = current_folder.split('\\')
    # print(split_)    
    Name_Prefix_ = Name_Prefix + ('_').join(split_[-2:])
    save_path = os.path.join(DATA_FOLDER, TEST_GROUP, 'Results', Name_Prefix_ + '_Results' + suffix)
    # save_path = os.path.join(DATA_FOLDER, TEST_GROUP, 'Results', Name_Prefix_ + '_Results' + suffix, matched_folder)
...
def main():
    global g_config, Name_Prefix, folder
    ...
    for seq_ in SEQUENCES:
        ...
        for deg_ in Degrees:
            ...
            for lamp_pos in LAMP_POSITIONS:
                log_(f'Processing for Lamp Position: {os.path.basename(lamp_pos)}...', 'magenta', 'italic')
                folders = [os.path.join(lamp_pos, f_) for f_ in os.listdir(lamp_pos) if os.path.isdir(os.path.join(lamp_pos, f_))]
                folder = lamp_pos
                for f in folders:
                    process_folder(f)
                    a=1
