...
from key_functions import get_json_file, get_illuminant_from, to_float64, to_uint8, check_memory, free_memory
...
SEQUENCES_ = [
    ...
    [False, True, True, True, False],  # sequence 2
]
...
run_predict, show_predict, save = False, False, True
Name_Prefix = ""
...
json_data = get_json_file(folder)
...
def process_folder(f_):
    ...
    f_bn = os.path.basename(f_)
    if not os.path.exists(f_):
        log_(f'Folder "{f_bn}" not found', 'red', 'italic', 'warn')
        return


    current_folder = f_
    log_(f'Processing Folder: "{current_folder}"...', 'magenta', 'italic')
    ...
    save_path = os.path.join(folder, 'Results', Name_Prefix + 'Results' + suffix)
    ...
    image_files = [file for ext in ("*.jpg", "*.png") for file in glob.glob(os.path.join(current_folder, ext))]
    ...
    if CC_Images:
        ...
        if CC_good:
            ...
            log_(f'No valid color correction results for folder "{f_bn}".', 'magenta', 'italic')
    else:
        log_(f'No color correction images found in folder "{f_bn}".', 'magenta', 'italic')
    ...
    if run_predict and Other_Images_:
        if CC_pred is not None:
            ...
            log_(f'Skipping inference because no valid CC prediction is available for folder "{f_bn}".', 'magenta', 'italic')
    else:
        log_(f'Skipping inference for folder "{f_bn}".', 'magenta', 'italic')
...
def main():
    ...
    for seq_ in SEQUENCES:
        ...
        Name_Prefix = f"With_{'FFC_' * DO_FFC}{'GC_' * DO_GC}{'WB_' * DO_WB}{'CC_' * DO_CC}{'Sat_' * CHECK_SAT}"
